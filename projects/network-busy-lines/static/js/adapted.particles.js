...
ParticleSystem.prototype.requestConnections = function(_id){
	//return [1,2,3,4];
	var _pickNum = Math.round(Math.random() * 100);
	var _pList = [];
...
Particle.prototype.draw = function(){
	...
	if(Math.random() > .9){
		var _d = 15
		_tX += Math.random()*_d - Math.random()*_d;
		_tY += Math.random()*_d - Math.random()*_d;
	...
	if (connections.length == 0){
		...
		if (connections[0]){
			var _connectX = connections[0].x;
			var _connectY = connections[0].y;
			//connections[0]
